...
function retrieveTweets() {
    ...
    let params = {screen_name: '_angrbrd', count: 20};

    // Retrieve the last 20 tweets
    client.get('statuses/user_timeline', params, function(error, tweets, response) {
        if (error) {
            let errorStr = 'ERROR: Retrieving user tweets -- ' + error;
...
function spotifySong(song) {
    ...
    spotify.search({ type: 'track', query: search}, function(error, data) {
        if (error) {
            let errorStr1 = 'ERROR: Retrieving Spotify track -- ' + error;
            ...
            if (!songInfo) {
                let errorStr2 = 'ERROR: No song info retrieved, please check the spelling of the song name!';
...
function retrieveOBDBInfo(movie) {
    ...
    let queryStr = 'http://www.omdbapi.com/?t=' + search + '&plot=full&tomatoes=true';

    // Send the request to OMDB
    request(queryStr, function (error, response, body) {
        if ( error || (response.statusCode !== 200) ) {
            let errorStr1 = 'ERROR: Retrieving OMDB entry -- ' + error;
